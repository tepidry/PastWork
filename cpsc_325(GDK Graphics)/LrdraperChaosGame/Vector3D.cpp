...
//@ post returns the normalized object without changing original vector
//@ usage w = v.hat();
Vector3D Vector3D::hat() {
	double length = Vector3D::length();
	double newx, newy, newz;
	newx = x / length; newy = y / length; newz = z / length;
...
double Vector3D::distance(const Vector3D& p) const{
   return sqrt(pow((x-p.x), 2) +pow((x-p.y), 2)+ pow((z-p.z), 2));
}
