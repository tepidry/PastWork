...
C234tree::C234tree(const C234tree& rhs234tree)
{
   mroot = NULL;
   mparent = NULL;
   copyTree(mroot, rhs234tree.mroot);
   mparent = mroot;
}
...
void C234tree::writeTree(C234node* treep, int level, ofstream& outfile)
{
   if (treep != NULL)
   {
      //backward inorder
      
      writeTree(treep -> mptrs[3], level +1, outfile);
      if (level == 1)
      {
         outfile << right << setw(7) << " " << treep -> mdata[2] << endl;
            
         writeTree(treep -> mptrs[2], level + 1, outfile);
         outfile << left << setw(7 * level) << "root->" << treep -> mdata [1] << endl;
         
         writeTree(treep -> mptrs[1], level +1, outfile);
         outfile << right << setw(7) << " " << treep -> mdata[0] << endl;
         
      }else{ 
         outfile << right << setw(8 * level - 4) << " " << treep -> mdata[2] << endl;
         writeTree(treep -> mptrs[2], level + 1, outfile);
         outfile << right << setw(8 * level - 4) << " " << treep -> mdata[1] << endl;
           
         writeTree(treep -> mptrs[1], level +1, outfile);
         outfile << right << setw(8 * level - 4) << " " << treep -> mdata[0] << endl;
      }
      writeTree(treep -> mptrs[0], level + 1, outfile);
   }
...
void C234tree::writePretty (C234node* treep, int level)
{
   if (treep != NULL)
   {
      ...
      if (level == 1)
      {
         cout << right << setw(7) << " " << treep -> mdata[2] << endl;
            
         writePretty(treep -> mptrs[2], level + 1);
         cout << left << setw(7 * level) << "root->" << treep -> mdata [1] << endl;
         
         writePretty(treep -> mptrs[1], level +1);
         cout << right << setw(7) << " " << treep -> mdata[0] << endl;
         
      }else{ 
         cout << right << setw(8 * level - 4) << " " << treep -> mdata[2] << endl;
         writePretty(treep -> mptrs[2], level + 1);
         cout << right << setw(8 * level - 4) << " " << treep -> mdata[1] << endl;
           
         writePretty(treep -> mptrs[1], level +1);
         cout << right << setw(8 * level - 4) << " " << treep -> mdata[0] << endl;
      }
...
void C234tree::copyTree (C234node*& newtreep, C234node* oldtreep) 
{
   ...
   {
      // preorder traversal
      newtreep = new C234node (*oldtreep);
   ...
   throw (cexception)
{
   if (treeptr == NULL) {
      ...
      while ((*indexBranch).IsNotLeaf())  
      {
         ...
         mparent = indexBranch;
         indexBranch = (*mparent).GetNextChild(newItem);
      }
      if ((*indexBranch).IsFull())
      {
         split(indexBranch, newItem);
      ...
      } else if ((*indexBranch).IsInNode(newItem)) {
