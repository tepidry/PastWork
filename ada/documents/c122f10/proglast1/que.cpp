...
ostream& operator<< (ostream& output, const Que& rhsq)
{
   ...
   if (rhsq.mcount > 0)
   {
      ...
      {
         output << rhsq.mitems[(front + i) % MAXQUE].custNum << " , ";
         output << rhsq.mitems[(front + i) % MAXQUE].arrivalTime << " , ";
         output << rhsq.mitems[(front + i) % MAXQUE].transactionTime << " , " << endl;
      }
      cout << " = ";
      output << rhsq.mitems[(front + i) % MAXQUE].custNum  <<  " , ";
      output << rhsq.mitems[(front + i) % MAXQUE].arrivalTime  <<  " , ";
      output << rhsq.mitems[(front + i) % MAXQUE].transactionTime  << " <- rear " << endl;
   } else {
      output << "Empty" << endl;
   }
   return output;
