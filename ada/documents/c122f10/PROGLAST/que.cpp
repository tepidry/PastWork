...
//@                with each index slot containing the Item Type of and 
//@                int for the customer number and int for the arrival time,
//@                and an int for the transaction time.
...
//@   operations: create, destroy, make empty, check if empty,
//@               check if full, 
...
void Que::Enqueue(const QItemType& newItem, bool& ok)
{
   ok = (mcount < MAXQUE);
   if (ok)
   {
...
void Que::GetFront(QItemType& frontItem) const
{
...
ostream& operator<< (ostream& output, const Que& rhsq)
{
   ...
   if (rhsq.mcount > 0)
   {
      ...
      {
         output << " {" << setw(2) << rhsq.mitems[(front + i) % MAXQUE].custNum  <<  "*";
         output << setw(2) << rhsq.mitems[(front + i) % MAXQUE].arrivalTime << "*";
         output << setw(2) << rhsq.mitems[(front + i) % MAXQUE].transactionTime << "} ";
      }
      output << " {" << setw(2) << rhsq.mitems[(front + i) % MAXQUE].custNum  <<  "*";
      output << setw(2) << rhsq.mitems[(front + i) % MAXQUE].arrivalTime  <<  "*";
      output << setw(2) << rhsq.mitems[(front + i) % MAXQUE].transactionTime << "}";
