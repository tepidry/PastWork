...
RGBColor
RenderEngine::trace_ray(const Ray& ray, ShadeRec& sr, int rayDepth)
{
    Vector3D normal;
...
/* start of commenting out reflective contribution
                ...
                double cosVR = (viewDir * reflectDir); // cos of angle between reflected direction vector and view direction vector
                if (cosVR > 0 )      // tests to see if view and reflected light are within 90 degrees of each other.
                {
                    ...
                    RGBColor specular = pow(cosVR, m_ptr -> sp) * (m_ptr -> ks) * (m_ptr -> cs) * light.intensity * light.color;
                    pixel_color 	+= specular;
                }
end of commenting out reflective contribution*/
            }
       }
...
RenderEngine::calc_reflected(const Ray& ray, ShadeRec& sr, int rayDepth)
{
    ...
    Ray refRay(sr.local_hit_point, reflectedDir);

    RGBColor reflected_color;
    reflected_color = trace_ray(refRay, sr, rayDepth); // warning - this changes sr
