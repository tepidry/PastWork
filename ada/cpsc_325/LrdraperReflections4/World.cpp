...
World::build(void)
{
    ...
    sphere_ptr -> set_center(5, 9, -85);
    sphere_ptr -> set_radius(3.0);
    Material* mat_ptr = new Material();
    mat_ptr -> setAllColor(0, 0, 1);       // red
    mat_ptr -> kr = 2;
    sphere_ptr -> set_material(mat_ptr);
    add_object(sphere_ptr);

    sphere_ptr = new Sphere(Vector3D(10, 6, -85), 2);
    mat_ptr = new Material();
    mat_ptr -> setAllColor(1,0,0);
    mat_ptr -> kr = 2;
    sphere_ptr -> set_material(mat_ptr);
    add_object(sphere_ptr);

    Plane* plane_ptr = new Plane(Vector3D(-10, -3, -100), Vector3D(0, 1, -.01));
    mat_ptr = new Material();
    mat_ptr -> setAllColor(.5,.5,.5);
    mat_ptr -> kr = 2;
    plane_ptr -> set_material(mat_ptr);
    add_object(plane_ptr);

    CheckerPlane* checkPlane_ptr = new CheckerPlane(Vector3D(-10, -2.99, -100), Vector3D(0, 1, -.01));
    mat_ptr = new Material();
    mat_ptr -> setAllColor(0,.2,.2);
    mat_ptr -> kr = 2;
    checkPlane_ptr -> set_material(mat_ptr);
    add_object(checkPlane_ptr);

    Box* box_ptr = new Box(Plane(Vector3D(7, 3, -85), Vector3D(0, 0, 1)), 1);
    mat_ptr = new Material();
    mat_ptr -> setAllColor(0,1,1);
    mat_ptr -> kr = 0;
    box_ptr -> set_material(mat_ptr);
    add_object(box_ptr);

    Box* box_ptr2 = new Box(Plane(Vector3D(2, 6, -100), Vector3D(0, 0, 1)), 4);
    mat_ptr = new Material();
    mat_ptr -> setAllColor(0.5, 0, .5);
    mat_ptr -> kr = 2;
    box_ptr2 -> set_material(mat_ptr);
    add_object(box_ptr2);

    PointLight* light_ptr = new PointLight();
    light_ptr -> set_color(1,1,1);
    light_ptr -> set_intensity(5);
    light_ptr -> set_location(10,600,500);
    add_light(light_ptr);
