...
//date: Sept 5, 2012
//file: main.cpp
//folder: LrdraperFigures
//description: This program draws two geometric figures in a 720 by 720 pixel window. We are using a dynamic 3-D array that is declared as the global vaiable image to color each pixel.
//             The bottom left is a square that is 250x250 pixels.  The top right figure is a isosceles right triangle that is [(200x200)/2] pixels.
...
void drawTriangleRightAdjacent(int xStart, int yStart, int size)
{
        int x = xStart +size;
...
void drawTriangleLeftAdjacent(int xStart, int yStart, int size)
{
        int x = yStart +size;
...
void drawHypotenuse(int xStart, int yStart, int size)
{
    int count = yStart+size;
    for (int row = xStart; row <= yStart+size ; row++){
        for (int col = yStart; col <= count; col++) {
...
void makeTriangle()
{
    ...
    drawTriangleRightAdjacent(triangleStartX, triangleStartY, size);
    drawTriangleLeftAdjacent(triangleStartX, triangleStartY, size);
    drawHypotenuse(triangleStartX, triangleStartY, size);
}
...
int main(int argc, char *argv[])
{
    ...
    image = new GLfloat[HEIGHT * WIDTH * 3];
