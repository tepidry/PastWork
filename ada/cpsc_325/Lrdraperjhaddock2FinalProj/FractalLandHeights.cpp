...
FractalLandHeights::FractalLandHeights(int s)
{
    ...
    gridSize = pow(2, s);   // grid size is 2^s
    makeTerrain();
}
...
{
    heights = new GLfloat[ (gridSize + 1) * (gridSize + 1)];   //gridsize + 1 X gridsize+1

    //initialize all heights to 0
    for (int i = 0; i <= gridSize; i++)
        for (int j = 0; j <= gridSize; j++)
            heights[index(j, i)] = 0.0;

    //set four corners to height 0.1
    heights[index(0, 0)              ] = 0.1;
    heights[index(0, gridSize)       ] = 0.1;
    heights[index(gridSize, 0)       ] = 0.1;
    heights[index(gridSize, gridSize)] = 0.1;
...
{
    float r = 1.0 * rand() / RAND_MAX; //r is between 0 and 1 not 0 or 1
    return (float) (inc * r - inc / 2.); //roughly between 0 and inc/2
}
