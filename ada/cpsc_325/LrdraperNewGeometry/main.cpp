...
//Disk myDisk;  // generates the vertices, colors, normals of a cube.
Cylinder myCylinder;
...
init()
{
    ...
    //int csize = sizeof(myDisk.points);
    int csize = sizeof(myCylinder.points);
    glBufferData( GL_ARRAY_BUFFER, 2*csize,
                  NULL, GL_STATIC_DRAW );
    glBufferSubData( GL_ARRAY_BUFFER, 0, csize, myCylinder.points );
    glBufferSubData( GL_ARRAY_BUFFER, csize, csize, myCylinder.colors );
    ...
    program = InitShader( "/home/GONZAGA/yerion/ANEWGEOMETRY/LrdraperNewGeomety/vertex.glsl", "/home/GONZAGA/yerion/ANEWGEOMETRY/LrdraperNewGeomety/fragment.glsl" );
    glUseProgram( program );
    //cout << "after shader stuff" << endl;
...
display( void )
{
    ...
    //glDrawArrays( GL_TRIANGLES, 0, myDisk.numPoints);
    glDrawArrays( GL_TRIANGLES, 0, myCylinder.numPoints);
...
main( int argc, char **argv )
{
    ...
    //HERE IS THE PROBLEM: THESE TWO LINES MUST BE COMMENTED OUT FOR OpenGL Version 3.1
    //THEY ARE OK in OpenGL Version 3.2 (which my home machine is)

    //glutInitContextVersion( 3, 2 );
   // glutInitContextProfile( GLUT_CORE_PROFILE );

    glutCreateWindow( "Cylinder Test" );
