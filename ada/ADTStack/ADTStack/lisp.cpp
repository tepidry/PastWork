...
void getAndCheckExpression()
{
   ...
   char symbol;
   bool balanced = true;
   bool notFullStack = true;
   
   cout << "Enter an expression and press return -> ";
   cin.get(symbol);
   while (  notEndOfLine(symbol)                             )
   {
      process(LISP_stack, symbol, balanced, notFullStack);
      cin.get(symbol);
   }
/*   if (!notFullStack)
   {
...
//@param symbol is from the expression
//@param balanced tells whether the expression has balanced parentheses
//@param notFullStack tells whether Push was successful
//@usage process(LISP_stk, symbol, balanced);
void process (Stack& stack, char symbol, bool& balanced, bool& notFullStack)
{
   if (isOpen(symbol))
   {
      int ascii = symbol;
      stack.Push(ascii, notFullStack);
